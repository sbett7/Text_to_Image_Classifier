...
from SparseLshClass_Color import SparseLshClass
import random

db_directory = '../Data/Hashing/IMDB/'

positive_file_location = '/../aclImdb_v1(1)/aclImdb/train/pos/'
negative_file_location = '/../aclImdb/train/neg/'
...
iRows = [(64, 64), (48, 64), (128, 128), (128, 64)]
...
# index.build({'vals': index.get_data_np(), 'nbits': 64})
...
for config in configuration:
    ...
    for iRow in iRows:
        for nbit in nbits_array:
            print("Config: {}       iRow: {}     nBit: {}".format('{}/{}bit/({}, {})/{}/database.csv'.
                                                                  format(db_directory, nbit, iRow[0], iRow[1], path),
                                                                  iRow, nbit))
            image_rows = iRow
            nbits = nbit

            hdidx = SparseLshClass(text, 100, image_shape=image_rows, hash_size=nbits, num_hashtables=1,
            ...
            for value in ran:
                ...
                query = (hdidx.get_query_code(data[0], data[1], 0, image_shape=image_rows,
                                              gray_scale=isGray, full_image=isFull, color_image=isColor))
                print(query)
                database.write("[{}], {}\n".format(query, int(data[1])))
